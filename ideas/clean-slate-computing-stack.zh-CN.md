...
[English](clean-slate-computing-stack.md)` | 中文`
...
**编程 / 应用层**
默认**内存安全语言**。所有程序运行在**可移植的中间表示**上(WASM 的思想),一次编译、跨任意 ISA 无缝运行——这会彻底消灭今天 x86→ARM 之间的移植噩梦。每个应用必须**显式声明它需要的能力**(网络?摄像头?哪个数据仓?),系统据此沙箱化,用户一眼能看清一个 App 到底能碰什么。
